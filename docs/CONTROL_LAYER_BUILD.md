...
> **Estimator amendment — 2026-09-06.** `docs/ACQUISITION_LOOP.md` fixes the
> acquisition method as a frozen generic `ChannelSchema`, one estimator per
> immutable column-incidence stream, and one paired numerical controller. The
> 4A--4E records below supply the structural history of Episode composition,
> nesting, one-unit processing, and typed end reasons. The active 4G correction
> retains per-column estimator state, reduces the complete normalized vector
> to marginal hypervolume as the sole method credit, and stops only on
> predicted future hypervolume credit. Coded is not evidence: live
...
| 4G-b — numerical controller migration and teardown | `acquisition-kernel` | **Correction in progress; live verification pending** | Foundational registered Firecrawl-plus-LLM experiment | The live `Episode` path consumes the complete per-column `IncidenceEstimate` vector through the versioned numerical controller. The controller derives marginal hypervolume as the sole method credit and uses predicted future hypervolume credit as its sole scalar stop statistic. Only the root may declare whole-run convergence. Coded is not evidence |
