...
> one-unit processing, and typed end reasons. 4G-a is Coded; live evidence
> waits for the atomic 4G-b controller migration and legacy teardown below.
...
| 4G-a — incidence rarefaction output | `acquisition-kernel` | **Coded** | §4G | `ChannelSchema` freezes generic base and kernel-derived union channels before observation; `IncidenceEstimator` records immutable eligible per-unit incidence sets and emits numeric role-based `IncidenceEstimate` records with exact rolling rarefaction and pairwise uncertainty plus bias-corrected incidence Chao2 reachable-total bands. Corrected post-code `modularity-steward` **PASS** and `reward-design-steward` **PASS**; invariant and deterministic implementation diagnostics pass. No live evidence; verification waits for atomic 4G-b controller migration and legacy teardown |
| 4G-b — numerical controller migration and teardown | `acquisition-kernel` + `modularity-steward` | **Chartered** | §4G | Wire the numerical controller to the upgraded `YieldCurve`; retire the replaced decision module, exports, configuration, and second loop/driver path once consumers move. Required-channel verdicts are conjoined; only the root can declare whole-run convergence |
