...
def load_graph(working_dir: str) -> nx.Graph:
...
def get_edge_phrase(graph: nx.Graph, u: str, v: str) -> str:
    if graph.has_edge(u, v):
        data = graph.get_edge_data(u, v) or {}
        rel = data.get("relationship_name") or data.get("description") or "connected to"
        rel = str(rel).strip().strip('"\'')
        rel_norm = rel.replace("_", " ").lower()
        if "inhibit" in rel_norm:
            return "inhibits"
        if "activat" in rel_norm or "upregulat" in rel_norm:
            return "activates"
        if "downregulat" in rel_norm:
            return "downregulates"
        if "cause" in rel_norm:
            return "causes"
        if "associat" in rel_norm:
            return "is associated with"
        return rel
    return "is not connected to"


def is_biological_node(graph: nx.Graph, node_id: str) -> bool:
    data = graph.nodes[node_id]
    etype = (data.get("entity_type") or data.get("type") or "").strip('"\'').lower()
    label = (data.get("label") or data.get("name") or "").strip('"\'').lower()
    bio_keywords = [
        "gene", "protein", "enzyme", "pathway", "compound", "antibiotic",
        "bacteria", "bacterial", "virus", "viral", "cell", "mic", "ic50",
        "inhibitor", "mutation", "phenotype", "metabolism", "growth"
    ]
    if any(k in etype for k in ["gene", "protein", "enzyme", "compound", "pathway", "phenotype"]):
        return True
    return any(k in label for k in bio_keywords)


def get_label(graph: nx.Graph, node_id: str) -> str:
    data = graph.nodes[node_id]
    raw = data.get("label") or data.get("name") or str(node_id)
    return clean_label(raw)


def get_type(graph: nx.Graph, node_id: str) -> str:
    data = graph.nodes[node_id]
    return data.get("type") or data.get("entity_type") or "UNKNOWN"


def neighbors_set(graph: nx.Graph, node_id: str) -> set:
    try:
...
def try_build_puzzle(graph: nx.Graph, rng: random.Random) -> dict | None:
    # Pick a seed node with at least 3 neighbors and biological relevance
    candidates = [n for n in graph.nodes() if len(list(graph.neighbors(n))) >= 3 and is_biological_node(graph, n)]
    if not candidates:
        return None
    ...
    # Choose three distinct neighbor anchors
    rng.shuffle(nbrs)
    a, b, c = nbrs[:3]

    # Compute intersection-of-neighbors constraint: entity connected to both a and b but not to c
    inter = neighbors_set(graph, a) & neighbors_set(graph, b)
    inter = {n for n in inter if n != a and n != b and n != c and n != center}
    exclude_c = neighbors_set(graph, c)
    candidates_ans = list(inter - exclude_c)

    if len(candidates_ans) != 1:
        return None

    answer_node = candidates_ans[0]

    # Build minimal facts for scenario (deterministic, no LLM)
    facts = []
    rel_ab = get_edge_phrase(graph, answer_node, a)
    rel_bb = get_edge_phrase(graph, answer_node, b)
    facts.append(f"{get_label(graph, answer_node)} {rel_ab} {get_label(graph, a)}.")
    facts.append(f"{get_label(graph, answer_node)} {rel_bb} {get_label(graph, b)}.")
    facts.append(f"{get_label(graph, answer_node)} is not connected to {get_label(graph, c)}.")

    # Add 2-3 distractors: other nodes connected to only one of {a,b} or connected to c
    distractors = []
    one_side = (neighbors_set(graph, a) ^ neighbors_set(graph, b)) - {center, a, b, c, answer_node}
    only_c = exclude_c - {center, a, b, c, answer_node}
    pool = list(one_side | only_c)
    rng.shuffle(pool)
    for d in pool[:3]:
        rels = []
        if d in neighbors_set(graph, a):
            rels.append(f"{get_edge_phrase(graph, d, a)} {get_label(graph, a)}")
        if d in neighbors_set(graph, b):
            rels.append(f"{get_edge_phrase(graph, d, b)} {get_label(graph, b)}")
        if d in neighbors_set(graph, c):
            rels.append(f"{get_edge_phrase(graph, d, c)} {get_label(graph, c)}")
        if rels:
            facts.append(f"{get_label(graph, d)} {', and '.join(rels)}.")

    scenario = "SCENARIO:\n" + "\n".join(f"- {f}" for f in facts)
    question = (
        scenario
        + "\n\nQUESTION: Identify the single biological entity that is connected to both "
        + f"{get_label(graph, a)} and {get_label(graph, b)} but not connected to {get_label(graph, c)}."
    )

    return {
        "question": question,
        "answer": get_label(graph, answer_node),
        "pattern": "intersection_minus_exclusion",
        "anchors": {
            "include_1": get_label(graph, a),
            "include_2": get_label(graph, b),
            "exclude": get_label(graph, c),
        },
    }


def generate_logic_puzzles(graph: nx.Graph, num_questions: int, seed: int = 7) -> list[dict]:
    rng = random.Random(seed)
    puzzles: list[dict] = []
    attempts = 0
    max_attempts = num_questions * 20

    while len(puzzles) < num_questions and attempts < max_attempts:
        attempts += 1
        puzzle = try_build_puzzle(graph, rng)
        if puzzle is None:
            continue
        # Ensure uniqueness by (answer, anchors) signature
        sig = (
            puzzle["answer"],
            puzzle["anchors"]["include_1"],
            puzzle["anchors"]["include_2"],
            puzzle["anchors"]["exclude"],
        )
        if any(
            (
                p["answer"],
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate logic-puzzle-style short-answer questions deterministically from the graph."
    )
    parser.add_argument("--working-dir", type=str, required=True, help="Path to working directory containing graphrag_cache.")
    parser.add_argument("--output-file", type=str, default="logic_puzzle_qa.json", help="Output JSON file for QA pairs.")
    parser.add_argument("--num-questions", type=int, default=10, help="Number of questions to generate.")
    parser.add_argument("--seed", type=int, default=7, help="Random seed for reproducibility.")
    args = parser.parse_args()

    graph = load_graph(args.working_dir)
    print(f"Graph loaded with {graph.number_of_nodes()} nodes and {graph.number_of_edges()} edges.")

    puzzles = generate_logic_puzzles(graph, args.num_questions, seed=args.seed)
    print(f"Generated {len(puzzles)} logic puzzles out of target {args.num_questions}.")
    ...
    all_items = existing + puzzles
    with open(args.output_file, "w") as f:
        json.dump(all_items, f, indent=2)

    print(f"Saved {len(puzzles)} new items to {args.output_file}. Total: {len(all_items)}")
