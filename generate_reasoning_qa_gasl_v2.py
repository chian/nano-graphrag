...
- Meta-questions about publications, errata, volumes, DOIs, or author information
...
async def enrich_entity_descriptions(graph: nx.Graph, entity_id: str, label_field: str, desc_field: str):
    """Get richer context for an entity by examining its neighborhood."""
    if entity_id not in graph.nodes:
        return ""

    node_data = graph.nodes[entity_id]
    neighbors = list(graph.neighbors(entity_id))[:3]

    # Build context from relationships
    context_parts = []
    for n in neighbors:
        edge_data = graph.get_edge_data(entity_id, n)
        rel_desc = edge_data.get('description', 'connected to') if edge_data else 'connected to'
        neighbor_label = create_clean_entity_label(graph.nodes[n], label_field, desc_field)
        context_parts.append(f"{rel_desc[:50]} {neighbor_label[:50]}")

    return " | ".join(context_parts)

async def multi_hop_gasl_exploration(graph: nx.Graph, llm: ArgoBridgeLLM, idea: dict, max_hops: int = 3):
    """
    NEW: Use GASL to perform multi-hop exploration, building up a richer knowledge state.

    This creates more complex reasoning by:
    1. Starting with seed entities related to the biological focus
    2. Following relationships across multiple hops
    3. Identifying causal chains, pathways, or mechanisms
    4. Building a richer compiled state with multi-step reasoning paths
    """
    ...
    print(f"🔍 Multi-hop GASL exploration for: {biological_focus}")

    # Create temporary state file
    state_file = tempfile.mktemp(suffix='.json', prefix='gasl_multihop_')
    adapter = NetworkXAdapter(graph)
    executor = GASLExecutor(adapter, llm, state_file=state_file)

    try:
        # Step 1: Find seed entities
        seed_query = f"Find all entities directly related to {biological_focus}. Store them in a variable called seed_entities."
        print(f"  Hop 0: Finding seed entities...")
        seed_result = executor.run_hypothesis_driven_traversal(seed_query, max_iterations=3)

        # Step 2: For causal/mechanistic questions, trace causal chains
        if reasoning_type in ['causal', 'mechanistic', 'temporal']:
            chain_query = f"Starting from the seed_entities related to {biological_focus}, trace causal relationships and mechanisms. Find entities that are upstream causes and downstream effects. Store the causal chain in causal_pathway."
            print(f"  Hop 1: Tracing causal chains...")
            chain_result = executor.run_hypothesis_driven_traversal(chain_query, max_iterations=4)

        # Step 3: For comparative questions, find contrasting entities
        elif reasoning_type == 'comparative':
            contrast_query = f"Starting from entities related to {biological_focus}, find contrasting or alternative mechanisms, compounds, or approaches. Store them in comparison_set."
            print(f"  Hop 1: Finding contrasts...")
            chain_result = executor.run_hypothesis_driven_traversal(contrast_query, max_iterations=4)

        # Step 4: For synthesis questions, gather interconnected components
        elif reasoning_type == 'synthesis':
            synthesis_query = f"Starting from entities related to {biological_focus}, find all interconnected components, pathways, and mechanisms that work together. Store the integrated system in synthesis_components."
            print(f"  Hop 1: Gathering system components...")
            chain_result = executor.run_hypothesis_driven_traversal(synthesis_query, max_iterations=4)

        else:
            # Default: just expand the neighborhood
            expand_query = f"Starting from seed_entities related to {biological_focus}, expand to find related entities within 2 hops. Store expanded entities in expanded_set."
            print(f"  Hop 1: Expanding neighborhood...")
            chain_result = executor.run_hypothesis_driven_traversal(expand_query, max_iterations=3)

        # Step 5: Extract the final compiled state
        final_result = chain_result if 'chain_result' in locals() else seed_result

        print(f"✅ Multi-hop exploration complete")
        return final_result
...
async def compile_state_for_idea(graph: nx.Graph, llm: ArgoBridgeLLM, idea: dict, graph_samples: list = None, use_multihop: bool = True):
    ...
    # NEW: Use multi-hop exploration for deeper reasoning
    if use_multihop:
        gasl_result = await multi_hop_gasl_exploration(graph, llm, idea, max_hops=3)
    else:
    ...
    # Extract entities and relationships from GASL result
    compiled_state = extract_state_from_gasl_result(gasl_result, idea, graph)
...
def extract_state_from_gasl_result(gasl_result: dict, idea: dict, graph: nx.Graph):
            ...
            if var_data.get('_meta', {}).get('type') == 'LIST':
                ...
                for item in items:
                    if isinstance(item, dict):
                        # Create entity from GASL result with enriched context
                        entity = create_entity_from_gasl_item(item, var_name, graph)
                        if entity:
    ...
    # REMOVED: No more node_X mapping
    # Instead, entities use their full labels directly
    ...
    for var_name, var_data in variables.items():
        if isinstance(var_data, dict):
            ...
            relationships = extract_relationships_from_gasl_variable(var_data, compiled_state['entities'])
    ...
    if not compiled_state['relationships'] and len(compiled_state['entities']) > 1:
        compiled_state['relationships'] = infer_relationships_from_entities(compiled_state['entities'], graph)

    return compiled_state

def create_entity_from_gasl_item(item: dict, source_var: str, graph: nx.Graph = None):
    """Create an entity from a GASL result item with enriched context."""

    # Extract ID
    entity_id = item.get('id', f"gasl_{source_var}_{hash(str(item))}")

    # Create clean label - this is what will be used in questions (no node_X!)
    label = create_clean_entity_label(item, 'name', 'description')

    entity = {
        'id': entity_id,
        'label': label,  # This is the ONLY label used in questions
        'source': 'gasl',
    ...
    # NEW: Enrich with neighborhood context if graph is available
    if graph and entity_id in graph.nodes:
        # Get label and desc fields dynamically
        available_fields = graph.nodes[entity_id].keys()
        label_field = next((f for f in ['label', 'name', 'title'] if f in available_fields), None)
        desc_field = next((f for f in ['description', 'text', 'content'] if f in available_fields), None)
        ...
        if neighbors:
            ...
            for n in neighbors:
                edge_data = graph.get_edge_data(entity_id, n)
                rel = edge_data.get('description', 'related to') if edge_data else 'related to'
                n_label = create_clean_entity_label(graph.nodes[n], label_field, desc_field)
                neighbor_context.append(f"{rel[:40]} {n_label[:40]}")
            entity['context'] = " | ".join(neighbor_context[:2])  # Limit to 2 neighbors

    return entity

def extract_relationships_from_gasl_variable(var_data: dict, entities: list):
    """Extract relationships from a GASL variable."""

    relationships = []

    # Create entity lookup by ID
    entity_lookup = {e['id']: e['label'] for e in entities}
        ...
        for item in items:
            if isinstance(item, dict):
                ...
                if 'from' in item and 'to' in item:
                    ...
                    # Use full entity labels, not node_X
                    from_label = entity_lookup.get(from_id, from_id)
                    to_label = entity_lookup.get(to_id, to_id)
...
def infer_relationships_from_entities(entities: list, graph: nx.Graph = None):
    """Infer relationships between entities based on their content and graph structure."""

    relationships = []

    # If graph is available, use actual edges
    if graph:
        entity_ids = {e['id'] for e in entities if e['id'] in graph.nodes}
        for e1 in entities[:8]:  # Limit to avoid too many
            if e1['id'] not in graph.nodes:
                continue
            for e2 in entities:
                if e2['id'] not in graph.nodes or e1['id'] == e2['id']:
                    continue
                # Check if there's an edge
                if graph.has_edge(e1['id'], e2['id']):
                    edge_data = graph.get_edge_data(e1['id'], e2['id'])
                    rel_desc = edge_data.get('description', 'related to') if edge_data else 'related to'
                    relationships.append({
                        'from': e1['label'],
                        'to': e2['label'],
...
def share_biological_keywords(entity1: dict, entity2: dict):
    ...
    bio_keywords = [
        ...
        'strain', 'pathogen', 'microbial', 'phage', 'drug', 'mechanism', 'protein',
        'gene', 'enzyme', 'pathway', 'metabolism', 'synthesis'
    ]
...
async def generate_reasoning_question(llm: ArgoBridgeLLM, idea: dict, compiled_state: dict):
    ...
    # Format compiled state with NO node_X references
    ...
    for entity in compiled_state['entities'][:8]:
        ...
        context_info = f"\n  Context: {entity.get('context', '')}" if entity.get('context') else ""
        state_text += f"- {entity['label']}{type_info}{desc_info}{context_info}\n"
...
5. Provides exactly 8 options (A–H) with exactly one correct answer.
6. Stays strictly within the BIOLOGICAL FOCUS and uses only entities/types mentioned in COMPILED KNOWLEDGE.
7. Write the question in CLEAR, DIRECT language:
   - Limit conditional clauses to a maximum of 2
   - Use straightforward sentence structure
   - State the scenario, then ask ONE clear question
   - Avoid nested parentheticals and multiple "considering that..." clauses

CRITICAL REQUIREMENTS:
- Forbidden phrases: "the text", "the paper", "the study", "as mentioned", "Figure", "Table", "Section"
- NEVER use placeholder references like "node_1", "node_X", "entity_A", etc. ALWAYS use the full, meaningful entity names from the COMPILED KNOWLEDGE.
...
DISTRACTOR REQUIREMENTS (7 wrong answers):
Each distractor must:
- Be biologically plausible at first glance
- Contain a SUBTLE but definite error (not obviously wrong)
- Represent a common misconception or nearby reasoning path
- Not contradict basic biology that any expert would know
...
async def filter_question_quality(llm: ArgoBridgeLLM, question: str, compiled_state: dict):
    """Filter questions for quality with expanded auto-disqualifiers."""

    # EXPANDED auto-disqualifiers to catch meta-scientific questions and placeholder refs
    ...
    auto_disqualifiers = [
        ...
        # NEW: Meta-scientific disqualifiers
        "erratum", "volume", "issue", "doi", "10.1128",
        "correspondence", "author responsibility", "publication",
        "supplemental file", "supplemental material",
        # NEW: Placeholder disqualifiers
        "node_", "entity_", "_node", "_entity"
    ]

    if any(p in lower_q for p in auto_disqualifiers):
        disqualifier = next(p for p in auto_disqualifiers if p in lower_q)
        return {"score": 0.0, "critique": f"auto-disqualified ({disqualifier})", "passed": False}
...
"""
    ...
    try:
        ...
        print(f"Warning: Could not parse filter response: {response[:200]}")
        return {"score": 0, "critique": f"Parse error: {e}", "passed": False}

async def check_circular_reasoning(llm: ArgoBridgeLLM, question: str):
    """NEW: Check if question has circular reasoning where answer is in the stem."""
    prompt = f"""Does this question exhibit circular reasoning where the answer is directly stated in or trivially derivable from the question stem itself?

{question}

Answer with ONLY 'YES' if circular, 'NO' if not circular."""

    response = await llm.call_async(prompt)
    return 'yes' in response.lower()

async def verify_biological_reasoning(llm: ArgoBridgeLLM, question: str, bio_focus: str):
    """NEW: Verify that question actually tests biological reasoning, not meta-science."""
    prompt = f"""Does this question genuinely test biological/scientific reasoning related to "{bio_focus}"?

{question}

A question tests biological reasoning if:
- It requires understanding biological mechanisms, pathways, or processes
- The answer depends on scientific principles, not just factual lookup
- It involves causality, synthesis, or mechanistic thinking
- It does NOT focus on publication metadata (errata, volumes, DOIs, authorship)

Output JSON: {{"passed": true/false, "reason": "brief explanation"}}"""
    ...
    try:
        ...
        return {"passed": True, "reason": "parse error - default pass"}
...
async def main(args):
    ...
    while len(generated_qa_pairs) < args.num_questions and attempts < max_attempts:
        ...
        try:
            # Compile state from graph using multi-hop GASL exploration
            print("    - Compiling state from graph (multi-hop GASL)...")
            compiled_state = await compile_state_for_idea(graph, llm, idea, graph_samples, use_multihop=args.use_multihop)
            ...
            # NEW: Check for circular reasoning
            print("    - Checking for circular reasoning...")
            if await check_circular_reasoning(llm, question):
                print("    ❌ Rejected: Circular reasoning detected")
                continue

            # NEW: Verify biological reasoning
            print("    - Verifying biological reasoning...")
            bio_check = await verify_biological_reasoning(llm, question, idea['biological_focus'])
            if not bio_check['passed']:
                print(f"    ❌ Rejected: Not testing biological reasoning ({bio_check['reason']})")
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-file", type=str, default="reasoning_qa_v2.json", help="Path to save the generated QA pairs.")
    parser.add_argument("--num-questions", type=int, default=10, help="Number of reasoning questions to generate.")
    parser.add_argument("--use-multihop", action="store_true", default=True, help="Use multi-hop GASL exploration for deeper reasoning.")
