...
class AssertHandler(CommandHandler):
    """Handles ASSERT commands for postconditions."""
    
    def can_handle(self, command: Command) -> bool:
        return command.command_type == "ASSERT"
        ...
        try:
            ...
            if not exists:
                return self._create_result(
                    command=command,
                    status="error",
                    error_message=f"ASSERT failed: Variable {variable} does not exist"
                )
            ...
            if not condition_met:
                return self._create_result(
                    command=command,
                    status="error",
                    error_message=f"ASSERT failed: Condition '{condition}' not met for {variable}"
                )
            
            # Create provenance
            provenance = [
                self._create_provenance(
                    source_id="gasl-assert",
                    method="assert_check",
                    variable=variable,
                    condition=condition,
                    result=True
