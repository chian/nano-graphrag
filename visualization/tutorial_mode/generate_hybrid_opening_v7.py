...
GEN = ROOT / "generated" / "hybrid" / "opening_trial_v7"
...
def render_scene1():
    ...
    bg = Image.open(ASSETS / "01_eureka_myth_comic_v4.png").convert("RGBA")
    n = 5 * FPS
    for i in range(n):
        t = i / (n - 1)
...
def _jittered_ellipse_points(rect: tuple[int, int, int, int], seed: int, jitter: float = 5.0, n: int = 36):
    x0, y0, x1, y1 = rect
    cx = (x0 + x1) / 2
    cy = (y0 + y1) / 2
    rx = (x1 - x0) / 2
    ...
    for i in range(n):
        ...
        k = rng.uniform(-jitter * 0.5, jitter * 0.5)
        pts.append((cx + math.cos(a) * (rx + j), cy + math.sin(a) * (ry + k)))
    return pts


def speech_bubble(layer: Image.Image, text: str, rect: tuple[int, int, int, int], mouth: tuple[int, int], align: str = "left", seed: int = 0) -> None:
    x0, y0, x1, y1 = rect
    draw = ImageDraw.Draw(layer, "RGBA")
    shadow = Image.new("RGBA", layer.size, (0, 0, 0, 0))
    ds = ImageDraw.Draw(shadow, "RGBA")
    # drawn editorial balloon with irregular contour
    shadow_pts = [(px + 4, py + 5) for px, py in _jittered_ellipse_points(rect, seed + 77)]
    ds.polygon(shadow_pts, fill=(0, 0, 0, 56))
    shadow = shadow.filter(ImageFilter.GaussianBlur(6))
    layer.alpha_composite(shadow)

    fill = (251, 245, 225, 248)
    outline = (73, 54, 31, 255)
    pts = _jittered_ellipse_points(rect, seed)
    draw.polygon(pts, fill=fill)
    draw.line(pts + [pts[0]], fill=outline, width=4, joint="curve")
    inner = _jittered_ellipse_points((x0 + 6, y0 + 5, x1 - 6, y1 - 5), seed + 1, jitter=3.0)
    draw.line(inner + [inner[0]], fill=(145, 120, 85, 180), width=1, joint="curve")

    mx, my = mouth
    # proper pointed tail with slight wobble
    if mx < x0:
        p1, p2 = (x0 + 26, y1 - 24), (x0 + 44, y1 - 8)
    elif mx > x1:
        p1, p2 = (x1 - 44, y1 - 8), (x1 - 26, y1 - 24)
    else:
        p1, p2 = (mx - 14, y1 - 8), (mx + 14, y1 - 8)
...
def render_scene2():
    ...
    for i in range(n):
        ...
        if t < 0.28:
            speech_bubble(overlay, "It all fits!", (804, 108, 1088, 192), (886, 242), align="center", seed=11)

        # Better-positioned audience bubbles with mouth anchoring
        if 0.24 < t < 0.54:
            speech_bubble(overlay, "Fool!", (66, 134, 214, 206), (112, 332), align="center", seed=21)
        if 0.44 < t < 0.80:
            speech_bubble(overlay, "Continents\ndon't move!", (894, 120, 1192, 220), (1112, 520), align="center", seed=31)
        if t > 0.70:
            speech_bubble(overlay, "No basis\nin reality.", (80, 482, 314, 572), (86, 385), align="center", seed=41)
...
def stitch_review():
    ...
    subprocess.run([
        ...
        str(review / "01_opening_hybrid_trial_v7.mp4")
    ], check=True)
