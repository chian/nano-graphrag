...
from .demo_catalog import get_demo_catalog, get_demo, build_cinematic_demo_from_artifacts
...
def create_app(graph_path: Optional[str] = None,
    ...
    @app.route('/api/cinematic-demo')
    def get_cinematic_demo():
        """Build an on-demand long-form cinematic replay payload from committed run artifacts."""
        ...
        target_seconds_arg = request.args.get('target_seconds', '').strip()

        if not run_id or not qid:
            return jsonify({'error': 'run_id and qid are required'}), 400
        target_seconds = int(target_seconds_arg) if target_seconds_arg else None
